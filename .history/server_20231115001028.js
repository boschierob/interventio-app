...
app.get('/delete-intervention/:customerId/:interventionId', async (req, res) => {
  // Récupérer l'ID de l'employé depuis la session
  const employeeId = parseInt(req.cookies.employeeId);
...
async function updateDataFile(updatedData) {
  ...
  await fs.writeFileSync(dataPath, `module.exports = ${jsonData};`, 'utf-8');
