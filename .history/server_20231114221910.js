...
app.get('/delete-intervention/:customerId/:interventionId', (req, res) => {
  ...
  // Trouver et supprimer la date d'intervention par son identifiant unique
  const userInterventions = interventions[employeeId] || [];
  const updatedInterventions = userInterventions.filter(intervention => intervention.id !== interventionId);

  interventions[employeeId] = updatedInterventions;

  // Récupérer les données du fichier data.js
  const { customers, employees } = data;

  // Rechercher l'utilisateur (client) en fonction de son id
  const user = customers.find(customer => customer.employeeId === employeeId);

console.log(user.interventions);
  // Rechercher l'intervention dans les dates de l'utilisateur
  const interventionIndex = user.interventions.findIndex(intervention => intervention.id === interventionId);

  // Si l'intervention est trouvée, la supprimer
  if (interventionIndex !== -1) {
    user.interventions.splice(interventionIndex, 1);

    // Vous pouvez ensuite mettre à jour le fichier data.js si nécessaire
    // data.customers = customers;

    // Rediriger vers la page après la suppression
    res.redirect('/page');
  } else {
    // Si l'intervention n'est pas trouvée, peut-être gérer une erreur ou rediriger
