...
app.get('/page', (req, res) => {
  ...
  console.log(`employeeId session : ${req.session.employeeId}`);
...
app.post('/edit-intervention', (req, res) => {
  ...
  if (employeeId) {
    ...
    if (customer) {
        ...
        console.log(`edition reussie et employee id depuis la session : ${req.session.employeeId}`);
    } else {
        // Gérer le cas où le client n'est pas trouvé (peut-être afficher un message d'erreur)
        res.redirect('/page');
        console.log(`client non trouvé session employeeId : ${req.session.employeeId}`);
    }
} else {
    // Rediriger vers la page de connexion si l'employé n'est pas connecté
    res.redirect('/login');
    ...
    res.redirect('/page'); // Rediriger vers la page principale après l'édition
} catch (error) {
    console.error(error);
    res.redirect('/page'); // Rediriger vers la page principale en cas d'erreur
}
