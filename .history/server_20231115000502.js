...
app.get('/page', (req, res) => {
  ...
  if (employeeId) {
    ...
    const customer = customers.filter(customer => Array.isArray(customer.userId) ? customer.userId.includes(employeeId) : customer.userId === employeeId);

    // Rendre la page avec les données en tant que variables
    res.render('page', { employee, customer });
  } else {
